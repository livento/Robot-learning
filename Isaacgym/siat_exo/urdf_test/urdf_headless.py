...
class urdfTest(BaseTask):
    ...
    def create_sim(self):
        ...
        self.device = 'cpu'
        self.device_id = 0
        self.sim = self.gym.create_sim(self.device_id,-1,gymapi.SIM_PHYSX, sim_params)
    ...
    def test_stand(self):
        ...
        left_positions = []
        base_positions = []
        for dt in trange(trajectory.shape[0]):
            # step the physics
            self.gym.simulate(self.sim)
            self.gym.fetch_results(self.sim, True)
            self.gym.refresh_dof_state_tensor(self.sim)

            pos_action = torch.from_numpy(trajectory[dt])
            self.gym.set_dof_position_target_tensor(self.sim, gymtorch.unwrap_tensor(pos_action))
            ...
            right_positions.append(body_positions[6])
            left_positions.append(body_positions[12])
            base_positions.append(body_positions[0])      
